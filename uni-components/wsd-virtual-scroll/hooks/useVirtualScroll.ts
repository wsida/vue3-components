...
export interface CustomVirtualScrollReturn {
  ...
  deleteCellItemByvIndex: (index: number, callback?: Function) => void;
  deleteCellItemByIndex: (index: number, callback?: Function) => void;
  updateCellItemByvIndex: (index: number, callback?: Function) => void;
  updateCellItemByIndex: (index: number, callback?: Function) => void;
  addCellItemByvIndex: (index: number, callback?: Function) => void;
  addCellItemByIndex: (index: number, callback?: Function) => void;
  getCellItemSizeByvIndex: (index: number) => number;
  getCellItemSizeByIndex: (index: number) => number;
  getCellItemCacheTotalByIndex: (index: number) => number;
...
export const isType = (val: unknown, type: string) =>
  Object.prototype.toString.call(val).toLowerCase() === `[object ${type}]`;
...
export default function useVirtualScroll(
  ...
  const finalScrollDirection = computed(
      () => unref(unref(props).scrollDirection) ?? "y"
  );
  const finalPrefixDistance = computed(
      () => unref(unref(props).prefixDistance) ?? 0
  );
  const finalAffixDistance = computed(
      () => unref(unref(props).affixDistance) ?? 0
  );
  const finalScrollOffset = computed(
      () => unref(unref(props).scrollOffset) ?? 0
  );
  const finalCellCols = computed(() => unref(props).cellCols || 1);
  const finalCellSize = computed(() => unref(props).cellItemSize || virtualCellSize.value || 0);
  const defaultDynamicRenderSize = computed(
      () => unref(finalViewCount) + 2 * unref(finalPreviewCount)
  );
  const normalizeData = computed(() => {
      return unref(unref(props).data).map((cellItem) => {
          const keyField = unref(props).keyField ?? "";
          const keyValue =
              keyField && cellItem[keyField]
                  ? cellItem[keyField]
  ...
  const finalDynamicRenderSize = computed(() =>
      unref(props).safeDynamicRender
          ? unref(defaultDynamicRenderSize)
          : unref(listCounts)
  );
   ...
   */
  // 获取元素高度 - 指定虚拟索引
  function getCellItemSizeByvIndex(vIndex: number): number {
      const aIndex = Math.max(0, aStartIndex.value + vIndex);
      return getCellItemSizeByIndex(aIndex);
  }

  function getCellItemSizeByIndex(aIndex: number): number {
      if (unref(props).cellSizeMode === "fixed") return unref(finalCellSize);
      return getCellItemCacheSize(aIndex);
  }

  // 动态高度专属 - 指定虚拟索引添加元素后更新
  function addCellItemByvIndex(vIndex: number, callback?: Function) {
      if (unref(props).cellSizeMode === "fixed") return;
      const aIndex = Math.max(0, aStartIndex.value + vIndex);
      addCellItemByIndex(aIndex, callback);
  }

  // 动态高度专属 - 指定数据索引添加元素后更新
  function addCellItemByIndex(aIndex: number, callback?: Function) {
      if (unref(props).cellSizeMode === "fixed") return;
      // TO: 增量更新-难处理，后续考虑，现在部分更新
      opacity.value = 0;
      const len = normalizeData.value.length;
      virtualList.value = normalizeData.value.slice(aIndex);

      getDynamicVirtualListSize(
          () => {
              updateTotalPadding();
              updateStartPadding();
              _updateVirtualList(true); // 会重新计算totalPadding
              opacity.value = 1;
              callback && callback();
          },
          aIndex,
  ...
  // 动态高度专属 - 更新指定虚拟索引
  function updateCellItemByvIndex(vIndex: number, callback?: Function) {
      if (unref(props).cellSizeMode === "fixed") return;
      const aIndex = Math.max(0, aStartIndex.value + vIndex);
      updateCellItemByIndex(aIndex, callback);
  }

  // 动态高度专属 - 更新指定数据索引
  function updateCellItemByIndex(aIndex: number, callback?: Function) {
      if (unref(props).cellSizeMode === "fixed") return;
      const forceRender =
          (aStartIndex.value && aIndex < aStartIndex.value) ||
          (aEndIndex.value && aIndex > aEndIndex.value);
      if (forceRender) {
          virtualList.value.push(normalizeData.value[aIndex]);
      } else {
          virtualList.value[aIndex - aStartIndex.value] =
              normalizeData.value[aIndex];
      }
      getDynamicVirtualListSize(() => {
          updateTotalPadding();
          if (forceRender) {
  ...
  // 动态高度专属 - 删除指定虚拟索引
  function deleteCellItemByvIndex(vIndex: number, callback?: Function) {
      if (unref(props).cellSizeMode === "fixed") return;
      const aIndex = Math.max(0, aStartIndex.value + vIndex);
      deleteCellItemByIndex(aIndex, callback);
  }

  // 动态高度专属 - 删除指定数据索引
  function deleteCellItemByIndex(aIndex: number, callback?: Function) {
      if (unref(props).cellSizeMode === "fixed") return;
      const forceRender = aStartIndex.value && aIndex < aStartIndex.value;
      const rowIndex = Math.floor(aIndex / unref(finalCellCols));
      startHeightCache.splice(rowIndex, 1);
      nextTick(() => {
          updateTotalPadding();
          if (forceRender) {
              vStartIndex.value -= 1;
  ...
  function getCellItemCacheSize(index: number) {
      ...
      if (!!cache) {
          ...
          cellItemSize = finalCellSize.value;
      }
  ...
  function updatevStartIndex(distance?: number) {
      const top = Math.max(
          0,
          (distance ?? unref(props).scrollDistance) -
              unref(finalPrefixDistance)
      );
      let total = 0;
      let sid = 0;
      let start = 0;
  ...
  function updatevEndIndex() {
      vEndIndex.value = Math.min(
          unref(listCounts),
          unref(vStartIndex) + unref(finalViewCount)
      );
      updateaEndIndex();
  }
  ...
  function forceUpdateVirtualList() {
      virtualList.value = unref(normalizeData).slice(
          aStartIndex.value,
          aEndIndex.value
      );
  }
  ...
  function _updateVirtualList(force = false) {
      const shouldUpdate =
          force ||
          (!virtualList.value.length && !!listCounts.value) ||
  ...
  function getStartIndex(startIndex: number) {
      // startIndex - 数据索引
      let _index = Math.min(
          Math.max(0, startIndex - unref(finalPreviewCount)),
          unref(listCounts)
      );
  ...
  function getEndIndex(endIndex: number) {
      ...
      let _index = Math.max(
          0,
          Math.min(unref(listCounts), endIndex + unref(finalPreviewCount))
      );
  ...
  function scrollToIndex(index: number, callback?: Function) {
      // let startIndex = getStartIndex(index);
      let distance =
          getCellItemCacheTotal(index) + unref(finalPrefixDistance);
      let actualDistance = Math.max(
          0,
          distance -
              (unref(props).useAffixDistance
                  ? unref(finalAffixDistance)
                  : unref(finalScrollOffset))
      );
  ...
  function getDynamicVirtualListSize(
      callback?: Function,
      aIndex = -1,
      eIndex = -1
  ) {
      ...
      _renderTimeout = setTimeout(() => {
          const dynamicList =
              aIndex !== -1
                  ? eIndex !== -1
                      ? normalizeData.value.slice(aIndex, eIndex)
                      : [normalizeData.value[aIndex]]
                  : virtualList.value;
          ...
          query.exec((rects: UniApp.NodeInfo[]) => {
              ...
              let i = 0;
              let startIndex = aIndex !== -1 ? aIndex : aStartIndex.value;
              let total = getCellItemCacheTotal(startIndex);
              for (const rect of rects) {
                  if (rect) {
                      const index = startIndex + i;
                      const size =
                          unref(finalScrollDirection) === "x"
                              ? rect.width
                              : unref(finalScrollDirection) === "y"
                              ? rect.height
                              : 0;

                      registerCellItemCacheSize(index, size, total);
  ...
  watch(listCounts, () => {
      if (!unref(props).virtual) return;
      if (
          !unref(props).ignoreCountsChange &&
          unref(props).cellSizeMode === "fixed"
      ) {
          initVirtualList(
              unref(props).updateFormStart ? 0 : unref(innerScrollDistance)
          );
      }
  });
  ...
  return {
      ...
      getCellItemCacheTotalByIndex: getCellItemCacheTotal,
      addCellItemByvIndex,
      addCellItemByIndex,
      scrollTo,
      scrollToIndex,
