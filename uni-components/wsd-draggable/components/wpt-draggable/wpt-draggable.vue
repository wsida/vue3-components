<template>
  ...
  >
      ...
      >
          ...
          >
              <template :key="`${item[keyField]}-${index}`">
                  <slot
                      :item="item"
                      :index="index"
                  >
...
const dragItemStyle = computed<CSSProperties>(() => {
  let transformText = `translate(${dragOffset.left}px, ${dragOffset.top}px)`;
  if (!!props.moveScale) {
      transformText += ` scale(${Math.max(0.1, props.moveScale)})`;
  }
  return {
...
function onDrop($event: TouchEvent) {
  ...
  if (target !== -1 && source !== -1 && target !== source) {
      ...
      if (props.dragMode === "swap") {
          emits(
              "update:dataSource",
              swapArrayElements(props.dataSource, source, target)
          );
      } else if (props.dragMode === "move") {
          emits(
              "update:dataSource",
              moveElementInArray(
                  props.dataSource,
                  source,
                  Math.abs(target - source),
                  target - source > 0 ? "r" : "l"
              )
          );
      }
