...
export interface VirtualSwiperEmits {
    (event: "swiper-change", key: string | number, dataCurrent: number, swiperCurrent: number, init: boolean): void;
...
export default {
    ...
    watch: {
        dataCurrent: {
            handler(val) {
                this.$emit("current-change", this.currentKey, val, this.getPrevIndex(val, this.swiperCounts), this.getNextIndex(val, this.swiperCounts));
            },
            immediate: false,
        },
    },
    ...
    methods: {
        ...
        onSwiperChange(e: any) {
            if (this._swiperTimeout && (!this.ignoreChangeByManual || !this.changeManualFlag)) {
        ...
        // 更新渲染的swiper-item数据
        updateCurrentSwiper() {
            this.currentKey = this.currentSwipers[this.swiperCurrent][this.finalyKeyField];
            this.current = this.normalizeData.findIndex((item: SwiperItem) => item[this.finalyKeyField] === this.currentKey);
        ...
        updateCurrentSwiperByCircle() {
            ...
            if (this.swiperCounts < 3) {
                ...
                let newCurrentSwipers = [this.normalizeData[sIndex], this.normalizeData[cIndex], this.normalizeData[eIndex]];
        ...
        updateCurrentSwiperByDefault() {
            ...
            if (this.swiperCounts < 3) {
                ...
                if (cIndex > this.swiperStartOffset && cIndex < this.swiperEndOffset) {
                    ...
                    this.currentSwipers = this.normalizeData.slice(0, this.swiperMaxCounts);
        ...
        updateDataCurrent(index: number, trigger = true, init = false) {
            ...
            this.currentKey = this.normalizeData[this.dataCurrent][this.finalyKeyField];
            ...
            this._swiperEventTimeout = setTimeout(() => {
                trigger && this._swiperCurrentChange && this._swiperCurrentChange(this.currentKey, this.dataCurrent, this.swiperCurrent, init);
                trigger && this.$emit("swiper-change", this.currentKey, this.dataCurrent, this.swiperCurrent, init);
            }, this.defaultDuration + 1);
        },
        ...
        index2Current(index: number) {
            if (this.defaultCircular) {
                ...
                return index && index > this.swiperEndOffset ? ((index - this.swiperOffset) % 3) + this.swiperOffset : index % 3;
            }
        },
        ...
        initVirtualSwiper() {
            this.scrollIntoSwiper(this.getCurrentIndex(this.defaultCurrent, this.swiperCounts), !!this.triggerWhenMounted, true);
        },
    },
    ...
    expose: ["finalyKeyField", "finalyDuration", "finalyCircular", "currentKey", "dataCurrent", "swiperCurrent", "swiperCurrentTemp", "currentSwipers", "onSwiperChange", "scrollIntoSwiper", "index2Current"],
};
