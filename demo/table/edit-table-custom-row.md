...
<template>
  <RePageCard>
    <ReEditTable
      ...
      :columns="columns"
    >
      <template #action-slot="{ row, $index }">
        <template v-if="isEdit($index)">
          <el-button type="primary" link @click="handleSave($index)"
            >保存编辑</el-button
          >
          <el-button type="primary" link @click="handleCancel($index)"
            >取消编辑</el-button
          >
        </template>
        <template v-else>
...
function isEdit($index) {
  return editTableRef.value && editTableRef.value.existIndexEditRows($index);
}

function handleSave($index) {
  editTableRef.value &&
    editTableRef.value.validateRow(normalizeIndex($index), valid => {
      if (valid) {
        editTableRef.value && editTableRef.value.removeEditRows($index);
        editTableRef.value && editTableRef.value.removeRowCache($index);
      }
    });
}
